...
import { dictionaries as i18nDictionaries } from "@carpull/i18n";
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // In real app, load from SecureStore/AsyncStorage
    // For now default to de, but could detect device locale
  }, []);
  const handleSetLocale = (l: Locale) => {
    setLocale(l);
    // Persist would go here
  };
  return <Ctx.Provider value={{ locale, dict: dicts[locale], setLocale: handleSetLocale }}>{children}</Ctx.Provider>;
}
